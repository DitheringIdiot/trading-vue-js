...
export default class DCCore extends DCEvents {
    ...
    init_data($root) {
        ...
        if (!('onchart' in this.data)) {
            // Vue 2 | this.tv.$set(this.data, 'onchart', [])
            this.data.onchart = []
        }

        if (!('offchart' in this.data)) {
            // Vue 2 | this.tv.$set(this.data, 'offchart', [])
            this.data.offchart = []
        }

        if (!this.data.chart.settings) {
            // Vue 2 | this.tv.$set(this.data.chart,'settings', {})
        ...
        if (!('datasets' in this.data)) {
            // Vue 2 | this.tv.$set(this.data, 'datasets', [])
            this.data.datasets = []
        }
    ...
    update_ids() {
        ...
        for (var ov of this.data.onchart) {
            ...
            // Vue 2 | if (!ov.settings) this.tv.$set(ov, 'settings', {})
    ...
    merge_objects(obj, data, new_obj = {}) {
        ...
        // Vue 2 | this.tv.$set(obj.p, obj.i, new_obj)
        obj.p[obj.i] = new_obj
    ...
    merge_ts(obj, data) {
        ...
        if (o[1] >= o[0]) {
            ...
            if (!obj.v.length && !data.length) {
                // Vue 2 | this.tv.$set(obj.p, obj.i, od)
                obj.p[obj.i] = od
                return obj.v
            }
    ...
    fast_merge(data, point, main = true) {
        ...
        if (!data.length || upd_t > last_t) {
            ...
            // Vue 2 | if statement no longer necessary
            // if (main) {
